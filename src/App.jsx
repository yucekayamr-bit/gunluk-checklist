...
export default function App() {
  ...
  const deleteTask = async (id) => {
    await db().ref(`${rp()}/alltasks/${id}`).remove();
    await db().ref(`${rp()}/done/${selectedDate}/${myName}/${id}`).remove();
  ...
  const GroupSection = ({group,gIndex}) => {
    ...
    if(gTasks.length===0&&group) return null;
    const doneCount=gTasks.filter(t=>checked[t.id]).length;
